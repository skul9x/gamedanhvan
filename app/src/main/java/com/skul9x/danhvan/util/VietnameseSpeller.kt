...
object VietnameseSpeller {
    ...
    private val ONSETS = mapOf(
        ...
        "b" to "bờ",
        "h" to "hờ",
        "k" to "ca", // "k" is often read as "ca" in spelling context e.g. "ca a ca" -> "ka"? No, "k" goes with i, e, ê. "kờ"?
                     // "k" -> "ca" (like c) or "kờ". Let's use "kờ" to be safe/consistent with "cờ".
                     // Actually standard is "k" -> "ca", "c" -> "cờ". But "k" is used in "khi", "ke".
                     // Let's use "khờ" for "kh". For "k", let's use "ca" or "kờ". User example "khúc" -> "khờ".
                     // Let's use "kờ".
        "k" to "kờ",
        "l" to "lờ",
        "m" to "mờ",
    ...
    fun getSpelling(syllable: String): String {
        ...
        if (rhyme.isNotEmpty()) {
            if (rhyme.length > 1) {
                ...
                sb.append(combinedRhyme).append(", ") // "ư cờ ức, "
            }
        }
        ...
        if (onset.isNotEmpty()) {
            ...
            if (rhyme.isNotEmpty()) {
                sb.append(combinedRhyme).append(" ") // "trờ ức"
            }
            // Base sound
            sb.append(baseWithTone) // "trức"
        } else {
            // No onset (e.g. "án")
            // "a nờ án, sắc án"
